    ...
    while True:
        ...
        if user_input.lower() in ["quit", "exit"]:
            ...
            if not allocation_saved:
                save_session_profile(llm, conversation_history, existing_profile)
            break
        ...
        # ── Get Arjun's Response ──
        try:
            response = llm.invoke(messages)
            arjun_reply = response.content
            conversation_history.append(("assistant", arjun_reply))

            # ── Check if allocation was generated ──
            if "--- PORTFOLIO ALLOCATION (JSON) ---" in arjun_reply:
                # Split into conversational part and JSON part
                ...
                saved = save_session_profile(llm, conversation_history, existing_profile)
                if saved:
                    print("  ✅ Profile saved to user_profile.json")
                allocation_saved = True

                # ── BACKTESTER: auto-run for ETFs, ask for Mutual Funds ──
                if parsed is not None:
                    vehicle = _get_investment_vehicle(parsed)

                    if "etf" in vehicle:
                        # ETF user — run backtest automatically, no need to ask
                        print("─"*62)
                        print("  ETF portfolio detected — running backtest automatically.")
                        run_backtest_flow(parsed)

                    else:
                        # Mutual Fund user — ask if they want ETF-equivalent backtest
                        print("─"*62)
                        try:
                            do_backtest = input(
                                "  Want to see how this allocation would have performed\n"
                                "  over 5 years via equivalent ETFs? (yes/no): "
                            ).strip().lower()
                            if do_backtest in ["yes", "y", "haan", "ha"]:
                                run_backtest_flow(parsed)
                        except (KeyboardInterrupt, EOFError):
                            pass
                # ─────────────────────────────────────────────────────
