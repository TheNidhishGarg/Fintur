...
@tool
def run_portfolio_backtest(allocation_json: str) -> str:
    """
    Run a 5-year SIP backtest on the given portfolio 
    allocation JSON. Input must be the full allocation 
    JSON string as generated by Arjun. Returns backtest 
    results including total invested, final value, 
    XIRR, and ETF selections.
    """
    import json
    import re
    try:
        ...
        holdings = results.get("final_holdings", {})
        etf_list = ", ".join([
            f"{ac}: {h['ticker']}" 
            for ac, h in holdings.items()
        ])
        return json.dumps({
                "success": True,
                "summary": s,
                "final_holdings": holdings,
                "yearly_summary": results.get(
                    "yearly_summary", {}),
                "monthly_detail": results.get(
                    "monthly_detail", [])
            })
